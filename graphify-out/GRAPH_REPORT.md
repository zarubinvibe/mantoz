...
## Corpus Check
- 49 files · ~1,029,493 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 428 nodes · 521 edges · 42 communities (37 shown, 5 thin omitted)
- Extraction: 95% EXTRACTED · 5% INFERRED · 0% AMBIGUOUS · INFERRED: 28 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `91562730`
...
Cohesion: 0.09
Nodes (36): main(), plan_decision(), App-среда: персона решает в desktop-приложении через Harbor., Детерминированно выводит решение из доверия к технологии и приватности., _read_container_result(), run(), run_persona_trial(), _screenshot_evidence() (+28 more)
...
Cohesion: 0.21
Nodes (17): list_agents(), load_registry(), main(), resolve(), _exact_judge(), _extract_json(), judge_answer(), _live_judges() (+9 more)
...
Cohesion: 0.47
Nodes (11): check(), check_agent_registry(), check_attribution(), check_data_boundary(), check_grounding_registry(), check_parity_checklist(), check_persona_dag(), check_persona_schema() (+3 more)
...
Cohesion: 0.24
Nodes (5): DecisionApp, Минимальное desktop-приложение: диалог разрешения уведомлений., command(), main(), Проходит desktop-сценарий мышью и снимает виртуальный дисплей.
...
Cohesion: 0.33
Nodes (8): load_material(), main(), Детерминированная Chat-среда: реплики зависят от признаков персоны., Строит стабильный диалог из четырёх признаков персоны., run(), run_dialogue(), validate_transcript(), verify()
...
## Knowledge Gaps
- **195 isolated node(s):** `Largest size that still leaves a margin inside the plate.`, `Минимальное desktop-приложение: диалог разрешения уведомлений.`, `Среда Survey: персона отвечает на анкету детерминированно, по своим признакам.`, `Незавершённость тоже следствие персоны: бросающие на полпути не дописывают.`, `Прогоняет survey-задачу одной персоной ЧЕРЕЗ Harbor.      Ответы выводятся детер` (+190 more)
...
- **Why does `run_persona_trial()` connect `Community 7` to `Community 0`?**
  _High betweenness centrality (0.029) - this node is a cross-community bridge._
- **Why does `sample()` connect `Community 8` to `Community 0`, `Community 10`, `Community 28`, `Community 15`?**
  _High betweenness centrality (0.025) - this node is a cross-community bridge._
- **Why does `resolve_backend()` connect `Community 0` to `Community 16`, `Community 10`, `Community 7`?**
  _High betweenness centrality (0.023) - this node is a cross-community bridge._
- **What connects `Largest size that still leaves a margin inside the plate.`, `Минимальное desktop-приложение: диалог разрешения уведомлений.`, `Среда Survey: персона отвечает на анкету детерминированно, по своим признакам.` to the rest of the system?**
