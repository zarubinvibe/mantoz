#!/bin/sh
# Пробы среды App (T14). Вне paths исполнителя.
...
[ -f application/tasks/app-decision/task.toml ] || fail "нет контракта web-задачи"
ls application/tasks/app-decision/tests/* >/dev/null 2>&1 || fail "нет верификатора web-задачи"

# 2. Положительный контроль: прогон персоной даёт след действий и итоговое решение.
uv run python -m mantoz.app run --persona-seed 1 --out "$d/one.json" >/dev/null 2>&1 \
  || fail "прогон app одной персоной не отработал"
python3 -c "
import json
...
" || fail "результат app не содержит следа действий и решения"

# 3. Верификатор принимает честный результат.
uv run python -m mantoz.app verify --file "$d/one.json" >/dev/null 2>&1 \
...
"
if uv run python -m mantoz.app verify --file "$d/bad.json" >/dev/null 2>&1; then
  fail "верификатор принял результат без решения"
fi

# 5. Персона влияет на выбор: разные персоны - разные решения или следы.
uv run python -m mantoz.app run --persona-seed 2 --out "$d/two.json" >/dev/null 2>&1
python3 -c "
import json
...
" || fail "персона не влияет на поведение в приложении"

# 6. Задача самодостаточна для Harbor.
ls application/tasks/app-decision/environment/* >/dev/null 2>&1 \
...
" || fail "app выполняется мимо Harbor"
...
uv run python -m mantoz.population run --n-runs 8 --seed 9 --task app --out "$d/pop.json" >/dev/null 2>&1 \
  || fail "population-прогон по app не отработал"
python3 -c "
import json
...
" || fail "population-отчёт по app неполон"
...
# 11. Среда действительно computer-use, а не скрипт под видом приложения.
grep -qiE "computer-use|cua|xvfb|screenshot|desktop" application/tasks/app-decision/environment/Dockerfile \
  || fail "в образе нет computer-use рантайма - это не App-среда"

# 12. Тихий откат запрещён.
MANTOZ_SANDBOX=выдумка uv run python -m mantoz.app run --persona-seed 1 --out "$d/z.json" >/dev/null 2>&1 \
  && fail "при нерабочем бэкенде прогон успешен - есть тихий откат"

echo "пробы app: 12/12 ok"
